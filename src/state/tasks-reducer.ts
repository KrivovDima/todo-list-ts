...
export const addTaskTC = (todolistId: string, taskTitle: string) => (dispatch: Dispatch) => {
  todolistsAPI.createTask(todolistId, taskTitle)
    .then((res) => {
      ...
      dispatch(addTaskAC(taskTitle, todolistId, task));
    })
}
export const changeTaskStatusTC = (todolistId: string, taskId: string, status: TaskStatuses) => (dispatch: Dispatch, getState: () => AppRootStateType) => {
  const taskForCurrentTodolist = getState().tasks[todolistId].find(t => t.id === taskId);
  if (taskForCurrentTodolist) {
    const {id, todoListId, addedDate, order, ...model} = taskForCurrentTodolist;
    model.status = status;
...
export const changeTaskTitleTC = (todolistId: string, taskId: string, newTaskTitle: string) => (dispatch: Dispatch, getState: () => AppRootStateType) => {
  const taskForCurrentTodolist = getState().tasks[todolistId].find(t => t.id === taskId);
  if (taskForCurrentTodolist) {
    const {id, todoListId, addedDate, order, ...model} = taskForCurrentTodolist;
